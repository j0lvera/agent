...
task docker                  # Build container image (uses podman)
...
./bin/wise run "task" -w /path    # Run in specific directory

# Container
podman run -e OPENROUTER_API_KEY=... -e OPENROUTER_BASE_URL=... \
  -v /project:/work wise:latest run "your task" -w /work
...
- **config.toml**: Prompts and templates (searched in order: `./config.toml`, `/etc/wise/config.toml`)

The `{{.Task}}` placeholder in `user_prompt` gets replaced with the CLI argument. Container image has config baked into `/etc/wise/config.toml`.
